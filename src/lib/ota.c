...
int tcp_ota_start_server(int _port) {
    ...
    if (ssfd < 0) {
        OTA_LOGE("socket() error: %s", strerror(errno));
        return -1;
    }
    ...
    if (bind(ssfd, (struct sockaddr *)&server_address, sizeof(server_address)) < 0) {
        OTA_LOGE("bind() failed: %s", strerror(errno));
        return -1;
    }

    if (listen(ssfd, 1) < 0) {
        OTA_LOGE("listen() failed: %s", strerror(errno));
        return -1;
    }

    if (xTaskCreate(&server_task, "ota_tcp_server", SERVER_TASK_STACK_SIZE, NULL, tskIDLE_PRIORITY,
                    NULL) != pdPASS) {
        OTA_LOGE("xTaskCreate() failed");
        return -1;
    }
...
static void server_task(void *p) {
    ...
    while (1) {
        OTA_LOGI("listening on port %d", port);

        csfd = accept(ssfd, (struct sockaddr *)&ca, &caddr_len);
        int64_t start_time = esp_timer_get_time();
        if (csfd < 0) {
            OTA_LOGE("accept() failed: %s", strerror(errno));
            goto error_cont;
        }

        if (inet_ntop(AF_INET, &(ca.sin_addr), ca_str, INET_ADDRSTRLEN) == NULL) {
            OTA_LOGE("inet_ntop() failed: %s", strerror(errno));
            goto error_cont;
        }

        OTA_LOGI("update request from %s:%d", ca_str, ntohl(ca.sin_port));

        if (read_header() < 0) {
            OTA_LOGW("request has invalid header");
            goto error_cont;
        }

        OTA_LOGI("size of new firmware: %zu, md5: %s", header.size, header.md5str);
        OTA_LOGI("retreiving firmware...");
        if (update() < 0) {
            OTA_LOGE("failed to update, staying on the old version");
            goto error_cont;
        }

        float secs = (float)(esp_timer_get_time() - start_time) / 1e6f;
        OTA_LOGI("updated in %f secs", secs);

        write(csfd, OK_TCP_RESPONSE, sizeof(OK_TCP_RESPONSE) - 1);
        close(csfd);

        OTA_LOGI("new firmware is loaded, rebooting...");
        esp_restart();
...
static inline int update() {
    ...
    if (err != ESP_OK) {
        OTA_LOGE("esp_ota_begin() failed: %s", esp_err_to_name(err));
        return -1;
    }

    OTA_LOGI("writing firmware to %s", part->label);
    ...
    while (loaded < header.size) {
        ...
        if (n < 0) {
            OTA_LOGE("read() failed: %s", strerror(errno));
            goto err_abort_ota;
        }

        if (n == 0) {
            OTA_LOGE("expected %zu bytes of firmware, got %zu", header.size, loaded);
            goto err_abort_ota;
        }
        ...
        if (err != ESP_OK) {
            OTA_LOGE("esp_ota_write() failed: %s", esp_err_to_name(err));
            goto err_abort_ota;
        }
    }
    ...
    if (memcmp(header.md5, computed_md5, MD5_SIZE) != 0) {
        ...
        OTA_LOGE("md5 differs %s (given) != %s (computed)", header.md5str, computed_md5str);
        goto err_abort_ota;
    }
    ...
    OTA_LOGI("loaded %zu bytes in %f secs with avg speed %f b/s", header.size, secs, avg_speed);

    err = esp_ota_end(ota_handle);
    if (err != ESP_OK) {
        OTA_LOGE("esp_ota_end() failed: %s", esp_err_to_name(err));
        goto err_abort_ota;
    }
    ...
    if (err != ESP_OK) {
        OTA_LOGE("esp_ota_set_boot_partition() failed: %s", esp_err_to_name(err));
        return -1;
    }
